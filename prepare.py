...
the project-local datasets folder. The active dataset can be pinned with
...
ACTIVE_DATASET_PATH = str(DATASETS_ROOT / "active_dataset.txt")
...
def _tiny_legacy_parquet_paths(dataset_name=None):
    ...
    legacy_flat_data_dir = os.path.join(str(PROJECT_ROOT), "data")
    return (
...
def _download_tinystories_file(dataset_name):
    ...
    if url.startswith("https://huggingface.co/"):
        candidate_urls.append(url.replace("https://huggingface.co", HF_MIRROR_BASE_URL, 1))

    temp_path = filepath + ".tmp"
    last_error = None
    for candidate_url in candidate_urls:
        print(f"Data: downloading {filename} from {candidate_url}...")
        try:
            ...
            last_error = exc
            print(f"Data: download failed from {candidate_url}: {exc}")
            if os.path.exists(temp_path):
                os.remove(temp_path)

    raise RuntimeError(f"Could not download {filename} from any configured source.") from last_error
